...
enum EyeIndex { EyeLeft = 0, EyeRight = 1 };

enum ReprojectionMode { None, AlternateEyeReprojection, PreviousFrameReprojection, CombinedReprojection };

enum ImageType { Image, depth };
...
struct EvaluateParams {
    void* InCmdList = NULL; // optional, leave it NULL to use the built-in command list, which will execute immediately so better to submit
                            // your own command lists before calling
    ...
    TextureDesc* InUIColorAlpha = NULL; // optional, provide the UI and the plugin will render it according to the camera orientation
                                        // without the HMD rotaion and position affecting it.
    TextureDesc* OutEyeColor =
        NULL; // returns the texture containing the reprojected result, which is also the textures you got from calling init
...
struct __declspec(novtable) D3D12RendererAPI {
    ...
    virtual bool CreateTexture(int nWidth, int nHeight, DXGI_FORMAT format, D3D12_RESOURCE_STATES initialState, TextureDesc& textureDesc,
        bool isDepth, bool createUAV) = 0;
    virtual bool CreateFrameBuffer(int nWidth, int nHeight, FramebufferDesc& framebufferDesc, bool createDepth, bool createUAV) = 0;
    virtual void Blit(ID3D12GraphicsCommandList* cmdList, TextureDesc& srcDesc, TextureDesc& dstDesc, D3D12_VIEWPORT viewPort) = 0;
};
