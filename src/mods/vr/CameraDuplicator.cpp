...
void CameraDuplicator::on_pre_application_entry(void* entry, const char* name, size_t hash) {
    ...
    auto& vr = VR::get();
    {
        ...
        for (auto camera : cameras_to_remove) {
            vr->notify_camera_destroyed((RECamera*)camera);
    ...
    if (hash == "LockScene"_fnv) {
        if (!vr->is_hmd_active() || !vr->is_using_afw_foveated()) {
            std::scoped_lock _{ m_camera_mutex };
            for (auto camera : m_new_cameras) {
                ...
                if (m_new_to_old_camera.contains(camera)) {
                    auto old_game_object = utility::re_component::get_game_object((REComponent*)m_new_to_old_camera[camera]);

                    if (old_game_object != nullptr) {
                        old_game_object->shouldDraw = true;
                        old_game_object->shouldUpdate = true;
                    }
                }
            }
...
void CameraDuplicator::clone_camera() {
    ...
    const auto new_to_old_contains = m_new_to_old_camera.contains(main_camera);

    if (seen_contains && (old_to_new_contains || new_to_old_contains)) {
        return;
    }
    ...
    for (const auto& descriptor : m_wanted_components) {
        ...
        if (t_name_fnv == "via.render.LDRPostProcess"_fnv) {
            ...
            if (get_vignetting != nullptr && set_vignetting != nullptr) {
                ...
                const auto is_disable_vignette = true;
