...
// Must be included before using sol with managed objects (sol_lua_push).
...
template <typename Fn>
auto safe(Fn&& fn) -> std::optional<std::invoke_result_t<Fn>> {
    try {
        return fn();
    } catch (...) {
        return std::nullopt;
...
nlohmann::json pose_to_json(const RE4VR::StoredPose& pose) {
    nlohmann::json j;
...
std::optional<RE4VR::StoredPose> pose_from_json(const nlohmann::json& j) {
    if (!j.is_object() || !j.contains("joints") || !j["joints"].is_object()) {
        return std::nullopt;
    }
    RE4VR::StoredPose pose{};
...
const std::array<const char*, 6> RE4VR::SCOPE_WIDS{"4202", "4400", "4401", "4402", "6105", "6114"};
const std::array<const char*, 4> RE4VR::SCOPE_STATES{"ironsight", "normal", "thermal", "hipower"};
const std::array<const char*, 7> RE4VR::PIN_JOINTS{"Hip", "Spine_0", "Spine_1", "Spine_2", "Neck_0", "Neck_1", "Head"};
const std::array<const char*, 4> RE4VR::SPINE_YAW_JOINTS{"Spine_1", "Spine_2", "Neck_0", "Neck_1"};
const std::unordered_set<std::string> RE4VR::UB_JOINTS{"Spine_0", "Spine_1", "Spine_2", "Neck_0", "Neck_1"};
const std::vector<const char*> RE4VR::STOP_NODES{
...
const std::vector<const char*> RE4VR::START_NODES{
...
const std::vector<const char*> RE4VR::RUN_START_NODES{
...
std::filesystem::path RE4VR::cfg_path() {
    return REFramework::get_persistent_dir() / "reframework" / "data" / "re4_vr" / "re4_vr_movement.json";
}

void RE4VR::init_default_scope_sets() {
...
std::optional<std::string> RE4VR::on_initialize() {
    ...
    if (m_player_cam_td != nullptr) {
        if (auto* m = m_player_cam_td->get_method("updateCameraPosition")) {
            g_hookman.add(m, &RE4VR::pre_update_camera_position, &RE4VR::post_update_camera_position);
            spdlog::info("[RE4VR] Hooked PlayerCameraController.updateCameraPosition");
        }
    }

    if (auto* jh = sdk::find_type_definition("chainsaw.MotionJackSuppliedHolder")) {
        if (auto* m = jh->get_method("setupJackLayer")) {
            g_hookman.add(m, &RE4VR::pre_setup_jack_layer, &RE4VR::post_setup_jack_layer);
            spdlog::info("[RE4VR] Hooked MotionJackSuppliedHolder.setupJackLayer");
        }
    }
...
void RE4VR::on_config_load(const utility::Config&) {
    // Nested movement settings live in re4_vr/re4_vr_movement.json, not re2_tweaks.ini.
    load_json();
    m_cfg.cam_body_rotate = false;
}

void RE4VR::on_config_save(utility::Config&) {
    save_json();
}
...
void RE4VR::on_lua_state_destroyed(sol::state&) {
    reset_runtime();
}

void RE4VR::install_lua_api(sol::state& lua) {
    lua["__re4_roomscale_get"] = [this]() { return m_cfg.roomscale; };
    lua["__re4_roomscale_set"] = [this](sol::object v) {
        if (v.is<bool>()) {
            m_cfg.roomscale = v.as<bool>();
        } else {
            m_cfg.roomscale = v.get_type() != sol::type::nil && v.get_type() != sol::type::none;
        }
        save_json();
    };
    lua["__vr_zbob_tau"] = sol::nil;
...
void RE4VR::on_frame() {
    ScriptProfileGuard guard("RE4VR", "on_frame", (uint64_t)VR::get()->get_frame_count());
    apply_grav_fix();
}

void RE4VR::on_pre_application_entry(void*, const char* name, size_t hash) {
    const auto call = std::string("on_pre_application_entry:") + (name ? name : "");
    const auto frame = (uint64_t)VR::get()->get_frame_count();
    switch (hash) {
    case "LockScene"_fnv: {
        ScriptProfileGuard guard("RE4VR", call, frame);
        apply_auto_center();
...
void RE4VR::on_application_entry(void*, const char* name, size_t hash) {
...
void RE4VR::load_json() {
    ...
    try {
        ...
        spdlog::error("[RE4VR] Failed to load {}: {}", path.string(), e.what());
    }
}

void RE4VR::save_json() {
    try {
        const auto path = cfg_path();
        ...
        spdlog::error("[RE4VR] Failed to save movement json: {}", e.what());
    }
}

void RE4VR::apply_json(const nlohmann::json& d) {
...
nlohmann::json RE4VR::dump_json() const {
    nlohmann::json j;
...
bool RE4VR::lua_not_false(std::string_view name) {
    LuaGuard g;
    auto* L = g.lua();
    if (!L) {
        return true;
...
std::optional<double> RE4VR::lua_number(std::string_view name) {
    LuaGuard g;
    auto* L = g.lua();
    if (!L) {
        return std::nullopt;
...
std::optional<std::string> RE4VR::lua_string(std::string_view name) {
    LuaGuard g;
    auto* L = g.lua();
    if (!L) {
        return std::nullopt;
...
void RE4VR::lua_set_bool(std::string_view name, bool v) {
    LuaGuard g;
    if (auto* L = g.lua()) {
        (*L)[std::string{name}] = v;
    }
...
void RE4VR::lua_set_number(std::string_view name, double v) {
    LuaGuard g;
    if (auto* L = g.lua()) {
        (*L)[std::string{name}] = v;
    }
...
void RE4VR::lua_set_string(std::string_view name, std::string_view v) {
    LuaGuard g;
    if (auto* L = g.lua()) {
...
void RE4VR::lua_set_nil(std::string_view name) {
    LuaGuard g;
    if (auto* L = g.lua()) {
...
bool RE4VR::call_killswitch_bool(const char* fn, bool default_v) {
    LuaGuard g;
    auto* L = g.lua();
    if (!L) {
        return default_v;
...
bool RE4VR::is_pin_release() {
    return call_killswitch_bool("is_pin_release", false);
}

bool RE4VR::is_crouch_active() {
    return call_killswitch_bool("is_crouch_active", false);
}
...
bool RE4VR::pure_gameplay_only() {
    ...
    if (!call_killswitch_bool("is_pure_gameplay", true)) {
        return false;
    }
...
::REManagedObject* RE4VR::character_manager() {
    if (!obj_ok(m_character_manager)) {
        m_character_manager = sdk::get_managed_singleton<::REManagedObject>("chainsaw.CharacterManager");
    }
    return obj_ok(m_character_manager) ? m_character_manager : nullptr;
}

::REManagedObject* RE4VR::player_context() {
...
::RETransform* RE4VR::get_body_transform() {
    auto* ctx = player_context();
    if (!obj_ok(ctx)) {
        return nullptr;
    }
    auto* body = safe([&] { return sdk::call_object_func_easy<::REGameObject*>(ctx, "get_BodyGameObject"); }).value_or(nullptr);
...
::REManagedObject* RE4VR::get_player_cam_controller() {
    if (!obj_ok(m_camera_system)) {
        m_camera_system = sdk::get_managed_singleton<::REManagedObject>("chainsaw.CameraSystem");
    }
    if (!obj_ok(m_camera_system)) {
        return nullptr;
    }
    auto* main = safe([&] { return sdk::call_object_func_easy<::REManagedObject*>(m_camera_system, "get_MainCameraController"); }).value_or(nullptr);
...
::REManagedObject* RE4VR::ground_adsorber(::REManagedObject** out_ctx) {
    auto* c = player_context();
    if (out_ctx) {
        *out_ctx = c;
    }
    if (!obj_ok(c)) {
...
::REManagedObject* RE4VR::get_motion_fsm(::REManagedObject** out_ctx) {
    auto* ctx = player_context();
    if (out_ctx) {
        *out_ctx = ctx;
    }
...
bool RE4VR::joint_alive(::REJoint* j) {
...
::REJoint* RE4VR::get_hip_joint(::RETransform* tf) {
    if (m_hip_joint) {
...
::REJoint* RE4VR::get_spine_joint(::RETransform* tf, const char* name) {
...
std::optional<float> RE4VR::flat_yaw_of(const glm::quat& rot) {
...
glm::quat RE4VR::spin_yaw_quat(float yaw) {
    const auto h = yaw * 0.5f;
    return glm::quat{std::cos(h), 0.0f, std::sin(h), 0.0f};
}

Vector3f RE4VR::quat_rotate_vec3(const glm::quat& q, const Vector3f& v) {
    return q * v;
}

glm::quat RE4VR::hmd_quat() {
    auto& vr = VR::get();
    return glm::quat{vr->get_transform(0)};
}

float RE4VR::hmd_wrap(float a) {
    if (a > glm::pi<float>()) {
...
bool RE4VR::quat_approx_equal(const glm::quat& a, const glm::quat& b) {
...
bool RE4VR::is_ub_joint(std::string_view name) {
    return UB_JOINTS.contains(std::string{name});
}

bool RE4VR::is_user_turning() {
    auto& vr = VR::get();
    const auto ax = vr->get_right_stick_axis();
    return std::abs(ax.x) > 0.1f;
}

void RE4VR::apply_hip_follow(::RETransform* tf, const glm::quat& root_yaw_quat) {
...
bool RE4VR::spinepin_resolve(::RETransform* tf) {
...
bool RE4VR::spinepin_capture(::RETransform* tf, std::vector<PinRel>& rel, std::vector<std::optional<glm::quat>>& par_inv, std::optional<PinRel>& null_rel) {
...
void RE4VR::spinepin_store(const std::vector<PinRel>& rel, const std::vector<std::optional<glm::quat>>& par_inv, const std::optional<PinRel>& null_rel, bool crouch) {
    StoredPose store{};
...
bool RE4VR::spinepin_restore(bool crouch, std::vector<PinRel>& rel, std::vector<std::optional<glm::quat>>& par_inv, std::optional<PinRel>& null_rel) {
...
void RE4VR::apply_spine_pin(bool can_capture) {
    if (!m_cfg.spine_pin) {
        m_spinepin_rel.reset();
        lua_set_bool("__vr_surge_bridged", false);
        return;
...
void RE4VR::apply_ub_lock(bool can_capture) {
    if (!m_cfg.ub_lock) {
        m_ub_lock_has_pose = false;
        m_ub_lock_has_rel = false;
        m_ub_lock_base.valid = false;
...
void RE4VR::apply_spine_yaw(::RETransform* tf) {
...
void RE4VR::apply_crouch_ub_z(bool capture_base) {
    if (!m_cfg.spine_pin) {
        return;
    }
    if (m_crouchpin_rel) {
...
void RE4VR::apply_crouch_pin(bool can_capture) {
    if (!m_cfg.spine_pin) {
        return;
    }
...
void RE4VR::apply_hard_yaw(bool sample) {
    if (!m_cfg.enabled) {
        m_owned_yaw.reset();
        return;
    }
...
void RE4VR::roomscale_recenter_events() {
...
void RE4VR::apply_hmd_body_follow() {
...
void RE4VR::roomscale_flush_body() {
    if (!m_cfg.roomscale) {
...
RE4VR::ScopeSlot* RE4VR::active_scope_slot() {
...
void RE4VR::apply_pose_freeze(bool on) {
    auto& vr = VR::get();
...
void RE4VR::apply_scope_freeze_late() {
    if (!lua_is_true("__re4_force_killswitch_scope")) {
        apply_pose_freeze(false);
        return;
    }
...
HookManager::PreHookResult RE4VR::pre_update_camera_position(std::vector<uintptr_t>& args, std::vector<sdk::RETypeDefinition*>&, uintptr_t) {
    if (args.size() > 1) {
...
void RE4VR::post_update_camera_position(uintptr_t&, sdk::RETypeDefinition*, uintptr_t) {
    pcall([] { get()->drive_hmd_yaw(); });
}

// ---- Anim skip / grav / lag / squeeze ----

bool RE4VR::apply_skip_nodes(const std::vector<const char*>& nodes, float frames, bool enable, bool overwrite_interp, ::REManagedObject** out_ctx) {
...
bool RE4VR::apply_stop_skip(bool enable) {
    ::REManagedObject* ctx = nullptr;
...
bool RE4VR::apply_start_skip(bool enable) {
...
bool RE4VR::apply_no_pivot(bool disable) {
    ::REManagedObject* ctx = nullptr;
    auto* mfsm = get_motion_fsm(&ctx);
    if (!obj_ok(mfsm)) {
        return false;
...
bool RE4VR::sqab_in_gimmick() {
    if (!obj_ok(m_camera_system)) {
        m_camera_system = sdk::get_managed_singleton<::REManagedObject>("chainsaw.CameraSystem");
    }
    auto* main = m_camera_system ? safe([&] { return sdk::call_object_func_easy<::REManagedObject*>(m_camera_system, "get_MainCameraController"); }).value_or(nullptr) : nullptr;
    auto* busy = main ? safe([&] { return sdk::call_object_func_easy<::REManagedObject*>(main, "get_BusyCameraController"); }).value_or(nullptr) : nullptr;
...
void RE4VR::sqab_tick() {
    ...
    std::optional<Vector3f> sq_exit;
    {
        LuaGuard g;
        if (auto* L = g.lua()) {
            sol::object ex = (*L)["__re4_sq_exit"];
            if (ex.is<sol::table>()) {
                auto t = ex.as<sol::table>();
                sq_exit = Vector3f{(float)t.get_or("x", 0.0), (float)t.get_or("y", 0.0), (float)t.get_or("z", 0.0)};
            }
        }
    }
    ...
    if (m_sqab_active && !a) {
        ...
        if (auto* L = g.lua()) {
            ...
            t["z"] = exitp.z;
            (*L)["__re4_sq_exit"] = t;
            (*L)["__re4_sq_end_t"] = now;
        }
        m_sqab_pin = false;
    }
}

bool RE4VR::sqab_is_retrigger() {
    const auto dt = now_clock() - lua_number("__re4_sq_end_t").value_or(-999.0);
    if (dt >= SQAB_RETRIG_WINDOW) {
        return false;
    }
    std::optional<Vector3f> ex;
    {
        LuaGuard g;
        if (auto* L = g.lua()) {
            sol::object o = (*L)["__re4_sq_exit"];
            if (o.is<sol::table>()) {
                auto t = o.as<sol::table>();
                ex = Vector3f{(float)t.get_or("x", 0.0), (float)t.get_or("y", 0.0), (float)t.get_or("z", 0.0)};
            }
        }
    }
...
HookManager::PreHookResult RE4VR::pre_setup_jack_layer(std::vector<uintptr_t>& args, std::vector<sdk::RETypeDefinition*>&, uintptr_t) {
    auto& self = *get();
...
void RE4VR::post_setup_jack_layer(uintptr_t& ret_val, sdk::RETypeDefinition*, uintptr_t) {
    auto& self = *get();
