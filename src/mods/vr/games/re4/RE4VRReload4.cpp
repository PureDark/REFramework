...
std::shared_ptr<RE4VRReload4>& RE4VRReload4::get() {
    static auto inst = std::make_shared<RE4VRReload4>();
    return inst;
}

HookManager::PreHookResult RE4VRReload4::pre_pump_mute(std::vector<uintptr_t>& args, std::vector<sdk::RETypeDefinition*>&, uintptr_t) {
...
void RE4VRReload4::post_nop(uintptr_t&, sdk::RETypeDefinition*, uintptr_t) {}

std::optional<std::string> RE4VRReload4::on_initialize() {
    g_mag.seed_ada();
    g_mag.load();
    if (!g_hooks) {
        if (auto* td = sdk::find_type_definition("soundlib.SoundManager")) {
            if (auto* m = td->get_method("postRequestInfo(soundlib.SoundManager.RequestInfo)")) {
                g_hookman.add(m, &RE4VRReload4::pre_pump_mute, &RE4VRReload4::post_nop);
            }
        }
        g_hooks = true;
    }
...
void RE4VRReload4::on_lua_state_created(sol::state& lua) {
    g_mag.seed_ada();
    g_mag.load();
...
void RE4VRReload4::on_lua_state_destroyed(sol::state&) {
    g_mag.reset();
}

void RE4VRReload4::on_frame() {
    ScriptProfileGuard guard("re4_vr_reload4_dlc.lua", "on_frame", re4vr::profile_frame());
    g_mag.on_frame();
}

void RE4VRReload4::on_pre_application_entry(void*, const char*, size_t hash) {
    if (hash != "LockScene"_fnv && hash != "BeginRendering"_fnv) {
        return;
    }
    ScriptProfileGuard guard("re4_vr_reload4_dlc.lua",
...
void RE4VRReload4::on_application_entry(void*, const char*, size_t hash) {
